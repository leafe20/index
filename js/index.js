...
window.onload = function () {
	...
	(function () {
		...
		function tab() {
			...
			if (now == 0) {
				...
				move(oUlSmall, {
					left: -(now - 2) * aLiSmall[0].offsetWidth
				});
			} else {
				move(oUlSmall, {
					left: -(now - 1) * aLiSmall[0].offsetWidth
				});
			}
	...
	// eg2 
	(function () {
		var oBox = document.querySelector('.cubic');
		var aClick = oBox.getElementsByTagName('div');

		var y = 0;
		var x = 0;
		var arr = ['practise/m.jd/m.jd.html', 'practise/m.Guokr/m.Guokr.html', 'practise/m.ctrip/m.ctrip.html', 'practise/css3/circle.html', 'practise/css3/page.html', 'practise/css3/turn.html']

		for (var i = 0; i < aClick.length; i++) {
			(function (index) {
				aClick[i].ondblclick = function () {
